...
_MEOW_MANAGE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$_MEOW_MANAGE_DIR/../core/common.sh"
source "$_MEOW_MANAGE_DIR/../ui/menu_simple.sh"

# Config file paths
readonly WHITELIST_CONFIG_CLEAN="$HOME/.config/meow/whitelist"
readonly WHITELIST_CONFIG_OPTIMIZE="$HOME/.config/meow/whitelist_optimize"
readonly WHITELIST_CONFIG_OPTIMIZE_LEGACY="$HOME/.config/meow/whitelist_checks"
...
${GRAY}Edit: ${display_config}${NC}"
    ...
    if [[ ${#selected_cache_patterns[@]} -gt 0 ]]; then
        ...
        local IFS=','
        export MEOW_PRESELECTED_INDICES="${preselected_indices[*]}"
    else
        unset MEOW_PRESELECTED_INDICES
    fi
    ...
    if [[ -n "$MEOW_SELECTION_RESULT" ]]; then
        local -a selected_indices
        IFS=',' read -ra selected_indices <<< "$MEOW_SELECTION_RESULT"
