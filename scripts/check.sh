...
echo -e "${BLUE}=== Meow Check (${MODE}) ===${NC}\n"

SHELL_FILES=$(find . -type f \( -name "*.sh" -o -name "cat" \) \
    -not -path "./.git/*" \
...
if [[ "$MODE" == "format" ]]; then
    ...
    if command -v goimports > /dev/null 2>&1; then
        ...
        goimports -w -local github.com/tw93/Meow ./cmd
...
if command -v shellcheck > /dev/null 2>&1; then
    if shellcheck cat bin/*.sh lib/*/*.sh scripts/*.sh; then
...
if ! bash -n cat; then
    echo -e "${RED}${ICON_ERROR} Syntax check failed (cat)${NC}\n"
    exit 1
fi
